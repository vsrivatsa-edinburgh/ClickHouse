...
struct SurfFilterHash
{
    ...
    static ColumnPtr hashWithColumn(const DataTypePtr & data_type, const ColumnPtr & column, size_t pos, size_t limit)
    {
        ...
        const ColumnPtr actual_col = SurfFilter::getPrimitiveColumn(column);
        const DataTypePtr actual_type = SurfFilter::getPrimitiveType(data_type);
    ...
    static void getDecimalTypeHash(const IColumn * column, ColumnUInt64::Container & vec, size_t pos)
    {
        ...
        if (unlikely(!index_column))
            throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column type was passed to the surf filter index.");
    ...
    static void getNumberTypeHash(const IColumn * column, ColumnUInt64::Container & vec, size_t pos)
    {
        ...
        if (unlikely(!index_column))
            throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column {} was passed to the surf filter index", column->getName());
        ...
        /// to be consistent, we need to convert Float32 to Float64 processing, also see: SurfFilterHash::hashWithField
